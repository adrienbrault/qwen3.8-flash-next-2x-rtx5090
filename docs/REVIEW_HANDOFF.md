...
Repository: `~/Developer/ai/qwen3.8-flash-next-rtx5090`
...
Location: [r363-enable.sh:102](bench/r363-enable.sh:102), plus the workload checks at the end of the script.
...
Location: [r363-enable.sh:41](bench/r363-enable.sh:41).
...
Location: [r364-hotvocab.sh:121](bench/r364-hotvocab.sh:121); restoration is later in the normal success path.
...
Location: [launch-flashnext.sh:247](scripts/launch-flashnext.sh:247).
...
Location: [probe.py:153](bench/probe.py:153).
...
Location: [hotvocab-greedy-capture.py:44](bench/hotvocab/hotvocab-greedy-capture.py:44).
...
Location: [probe.py:72](bench/probe.py:72), including the `--distinct` CLI description.
...
Location: [summarize.py:32](bench/summarize.py:32).
...
While preparing improvement pointers, I inspected the sibling [greedy-compare.sh](kubernetes-home/flan/lib/greedy-compare.sh). Its comments say empty directories never pass, but `greedy_hash` hashes an empty file listing into the nonempty SHA-256 prefix `e3b0c44298fc1c14`; `greedy_same` only checks that the resulting hash is nonempty and equal. Two existing empty directories therefore satisfy the predicate on the intended GNU/Linux tools. This is a source-level finding, not a live-host test, and was not included in the original nine.
...
Read [the profiler how-to](kubernetes-home/flan/patches/exllamav3/ported/profiler-howto.md) and [profile_decode.py](kubernetes-home/flan/patches/exllamav3/ported/profile_decode.py). These already exist in the private sibling repository. Their own validation statement is CPU/static only; no successful GPU profile is assumed by this handoff.
...
Read [the EP status assessment](kubernetes-home/flan/patches/exllamav3/expert-parallel-qwen4exp-status.md) and recheck the current QSA/PLE follow-up work before planning. The documented prerequisites include QSA transport, PLE transport, replica/output-selection policy and MTP adapters. Their current status must be reconciled across worktrees; a compiled prerequisite patch is not a working EP engine.
