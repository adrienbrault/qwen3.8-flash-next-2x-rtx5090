...
The greedy set runs 1 stream, so it covers verify rows 1 and 4 only. The draft policy served until 2026-09-25, `[[4, 3], [5, 2], [8, 1]]`, verifies at 4, 8, 12, 16, 15, 12, 14 and 16 rows with 1 to 8 active jobs, and drafts at 1 to 8 rows. A batch gate proves identity at every one of them:
...
- The policy served since 2026-09-25, `[[4, 3], [8, 2]]`, verifies at 18, 21 and 24 rows with 6, 7 and 8 jobs. R717 and R717b covered those rows with kernel parity at every row count from 17 to 32 (each MoE output equal to two served 16-row calls) and with in-process sequence hashes that are the same over four rounds at 6, 7 and 8 streams depth 2; at 16 rows and below the flags change nothing (P1 hashes with and without them identical in 36 of 36 cells, [R717, R717b](../bench/results/r717-rows32.md)). Moving 6 to 8 jobs from depth 1 to depth 2 changes which tokens share a verify step, so the served multi-stream output is compared against the served configuration's own variation rather than for identity: greedy output at 8 streams differs from 1 stream on 57 of 64 prompts, against 59 of 64 for the configuration it replaced.
