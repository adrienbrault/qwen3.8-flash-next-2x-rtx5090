...
| --- | --- | --- |
| `IMG=` | `tabbyapi:stack-r3` | which image to serve; a patch variant is A/B'd without editing the launcher. The chain is in [`docker/README.md`](../docker/README.md) |
...
| `EXL3_MOE_COOP_V3=3`, `EXL3_MOE_COOP_V3_MAP=2-4:2`, `EXL3_SHARED_EXPERT_EARLY=1` | the routed-expert MoE decode kernels (on top of `EXL3_MOE_COOP_V2=1`, K = 2 to 4). Mode 2 (moefast r1, served R701 to R716b): a cp.async weight ring, a one-slice activation prefetch, one acquire/release counter arrival per item and the narrow down stage merged per 128-column chunk. Mode 3 (moefast r2 kernels) runs at 1 and 5 to 16 rows; the MAP keeps mode 2 at 2 to 4 rows. `SHARED_EXPERT_EARLY` forks the side-stream shared expert before the router, so it runs beside kernel A instead of delaying kernel B. Bitwise-identical | [R700b](../bench/results/r700b-moefast.md), [R713](../bench/results/r713-moefast-r3.md), [R716b](../bench/results/r716b-stack-r3.md) |
