...
| --- | --- | --- |
| `IMG=` | `tabbyapi:stack-r2` | which image to serve; a patch variant is A/B'd without editing the launcher. The chain is in [`docker/README.md`](../docker/README.md) |
...
| `EXL3_HC_MIX_V3=1`, `EXL3_HC_MIX_V3_DOTS_B=2`, `EXL3_HC_MIX_V3_UP_B=8` | the int8 hyper-connection mixer's dots and up kernels through `hc_mix_v3.cu`: each int8 weight converted once per iteration instead of once per row, the iteration's loads issued before the first multiply, the up kernel's reduction as a 35-shuffle reduce-scatter. Same launches, grids and operand order: bitwise-identical. `DOTS_B` and `UP_B` cap the rows per CTA; the 8/8 default regressed at 8 streams, dots at 2 is served | [R698, R699](../bench/results/r698-hcfast.md), [R701](../bench/results/r701-stack-r2.md) |
| `EXL3_MOE_COOP_V3=2` | the routed-expert MoE decode kernels (on top of `EXL3_MOE_COOP_V2=1`, batches above one row, K = 2 to 4) with a cp.async weight ring, a one-slice activation prefetch, one acquire/release counter arrival per item and the narrow down stage merged per 128-column chunk; bitwise-identical | [R700b](../bench/results/r700b-moefast.md), [R701](../bench/results/r701-stack-r2.md) |

## Image

`tabbyapi:stack-r2` (`tabbyapi:slotfix-r1` plus the [`hcfast-r1`](../docker/overlays/hcfast-r1/) and [`moefast-r1`](../docker/overlays/moefast-r1/) overlays, built in the order in [`docker/overlays/stack-r2/README.md`](../docker/overlays/stack-r2/README.md), [R701](../bench/results/r701-stack-r2.md); `slotfix-r1` is `tabbyapi:stack-r1` plus [`slotfix-r1.patch`](../docker/overlays/slotfix-r1/slotfix-r1.patch), which returns a recurrent-state slot to the pool when state construction fails, [R676](../bench/results/r676-slotfix.md); `stack-r1` is `tabbyapi:bverify-r1` plus the `mtpnorm`, `mixstate` and `prefbatch` patches — MTP input-norm fusion, the fused int8 state-in-up mixer kernel and grouped accept-prefill batching): TabbyAPI pinned at `53da7919`, ExLlamaV3 v1.5.0, the R338 requeue token-count fix, and every layer in [`docker/README.md`](../docker/README.md). Each Dockerfile asserts the versions it builds on. The earlier overlay installers check the SHA-256 of every file they replace; slotfix-r1, hcfast-r1 and moefast-r1 apply their patch with `--fuzz=0` after a dry run, and each asserts a landing marker in the built image.
