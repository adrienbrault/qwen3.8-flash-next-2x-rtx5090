...
# Avoid constructing scalar tensor views solely to obtain rewind descriptor pointers.
...
def _rewind_index(value, size: int) -> int:
    ...
    if not -size <= value < size:
        raise IndexError(f"index {value} is out of bounds for dimension of size {size}")
    return value % size
...
class GDNLayerState:
    ...
    def rewind_conv_job(self, slot: int, last_history: int, num_tokens: int):
        ...
        cdim = self.module.conv_kernel_size
        p = self.conv_state.shape[-1] - num_tokens
        if _host_gap_rewind and self.conv_state.dim() == 3:
            t = self.conv_state
            shape, strides = t.shape, t.stride()
            row = _rewind_index(slot, shape[0])
            _rewind_index(0, shape[1])
            col = _rewind_index(p - cdim, shape[2])
            _rewind_index(0, shape[2])
            size = t.element_size()
            dst = t.data_ptr() + row * strides[0] * size
            return ext.ConvRewindJob(
                dst + col * strides[2] * size, dst, shape[1], cdim, strides[1]
            )
        return ext.ConvRewindJob(
    ...
    def rewind_state_job(self, slot: int, last_history: int, num_tokens: int):
        ...
        if num_tokens == 0:
            return None
        if _host_gap_rewind and self.recurrent_state.dim() == 5:
            t = self.recurrent_state
            shape, strides = t.shape, t.stride()
            row = _rewind_index(slot, shape[0])
            history = _rewind_index(last_history + 1 - num_tokens, shape[1])
            _rewind_index(0, shape[1])
            size = t.element_size()
            dst = t.data_ptr() + row * strides[0] * size
            return ext.StateRewindJob(
                dst + history * strides[1] * size, dst,
                t.numel() // shape[0] // shape[1],
            )
        return ext.StateRewindJob(
