#!/usr/bin/env bash
# densegemm r2 install (run by Dockerfile.box, or by hand inside a --rm build container without --gpus):
...
#   2. apply densegemm-r2.patch to the installed exllamav3 (dry run first, --fuzz=0, exit codes checked)
...
# r2: DGV2_NVCC_DEFS (the compile bisect's choice, e.g. "-DDGV2_LAMBDA_AI=1") is passed to nvcc for every TU (the
# served SASS identity check proves it does not touch served code), and step 4b fails the install when any 16-row
# gemm / mgemm twin's STACK exceeds its served shape's by more than 64 bytes (compile_bar.py; R710's r1 twins had
# 544-1832 bytes, which r1's LOCAL-only check did not see).
...
patch -p1 --fuzz=0 --batch --forward --dry-run -d "$pkg" < "$here/densegemm-r2.patch"
patch -p1 --fuzz=0 --batch --forward --no-backup-if-mismatch -d "$pkg" < "$here/densegemm-r2.patch"
...
build=$(mktemp -d)
echo "densegemm: nvcc defines from the compile bisect: '${DGV2_NVCC_DEFS:-}'"
( cd "$build" && DGV2_NVCC_DEFS="${DGV2_NVCC_DEFS:-}" TORCH_CUDA_ARCH_LIST=12.0 MAX_JOBS="${MAX_JOBS:-2}" nice -n 19 \
...
PY
# 4b. local memory of the twins: STACK (spills and demoted arrays), not LOCAL, against the served shapes
...
python3 "$here/compile_bar.py" --base "$out/base-res-usage.txt" --twins "$out/rebuilt-res-usage.txt" --sass "$out/rebuilt.sass" \
    --label "install${DGV2_NVCC_DEFS:+ $DGV2_NVCC_DEFS}" | tee "$out/compile-bar.txt" | sed 's/^/densegemm: bar /' || true
grep -q "^BAR .* PASS " "$out/compile-bar.txt" || { echo "densegemm: COMPILE BAR FAILED (a 16-row gemm/mgemm twin runs out of local memory)"; exit 1; }

python3 -c '
import torch, importlib.util as u
print("ext:", u.find_spec("exllamav3_ext").origin)
import exllamav3_ext as e
assert e.dense_v2_revision == 2, "dense_v2_revision"
...
echo "densegemm r2 landed: patch applied, exllamav3_ext rebuilt for sm_120, dense_v2_revision=2, served SASS identical, 32 twins, compile bar PASS (default off)"
