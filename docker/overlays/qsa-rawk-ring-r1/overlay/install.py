#!/usr/bin/env python3
"""Hash-pin and apply the qsa-rawk-ring round-1 patch (EXL3_QSA_RAWK_RING) to the installed exllamav3 package.

Aborts unless the patch and every touched file match manifest.json exactly, applies with fuzz 0 (a non-zero patch
exit code aborts), then checks every patched file against its pinned post-patch hash. The patch is Python only,
so the prebuilt extension stays in place.
...
def main() -> None:
    ...
    if sha256(source_patch) != manifest["patch_sha256"]:
        raise SystemExit("qsa-rawk-ring r1 patch hash mismatch")
