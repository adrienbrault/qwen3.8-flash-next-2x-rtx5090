#!/usr/bin/env python3
"""Hash-pin and apply the decode-kernels round-6 patch to the installed exllamav3 package.
...
def main() -> None:
    ...
    manifest = json.loads(locate("manifest.json").read_text())
    source_patch = locate("served-source.patch")
    if sha256(source_patch) != manifest["patch_sha256"]:
        raise SystemExit("round-6 patch hash mismatch")
