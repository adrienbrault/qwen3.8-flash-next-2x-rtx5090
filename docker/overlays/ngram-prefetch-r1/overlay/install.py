#!/usr/bin/env python3
"""Hash-pin and apply the ngram-prefetch-r1 patch (EXL3_NGRAM_PREFETCH2 / EXL3_NGRAM_TIMING) to
the installed exllamav3 package.

Aborts unless the patch and every touched file match manifest.json exactly, applies with fuzz 0
(a non-zero patch exit code aborts), then checks every patched file against its pinned post-patch
hash. The patch is Python only, so the prebuilt extension stays in place.
...
def main() -> None:
    ...
    parser.add_argument("--patch", default = "served-source.patch")
    args = parser.parse_args()

    manifest = json.loads(locate(args.manifest).read_text())
    source_patch = locate(args.patch)
    if sha256(source_patch) != manifest["patch_sha256"]:
        raise SystemExit("ngram-prefetch r1 patch hash mismatch")
    ...
    # Compiled bytecode of the replaced sources would be stale-checked by mtime anyway; drop it so the image
    # carries no bytecode of the served versions
    removed = []
