...
class BlockSparseMLP(BlockSparseMLP_CPU, Module):
    ...
    def _run_batch_recon(self, recon, y, fhs_ext, token_sorted, weight_sorted, expert_count_list, groups,
                         scratch = None, tables = None, prefill_nosync = False):
        ...
        for grp in groups:
            ...
            recon.run_group(
                ...
                out_slab = out_slab, prefill_nosync = prefill_nosync)
            handled.update(grp)
        return handled
        ...
        # Torch/C++/fused path
            ...
            if self.num_local_experts is None or self.num_local_experts > 0:
                ...
                if e3_active or (self.fused_mode_buffers is not None and num_tokens * top_k <= self.fused_rows):
                    ...
                    if _prefill_nosync_enabled(params):
                        from ..util.prefill_nosync import read_counts
                        expert_count_list = read_counts(expert_count)
                    else:
                ...
                if FUSED_DET and not e3_active and (fused_total or groups):
                    ...
                    if expert_count_list is None:
                        ...
                        if _prefill_nosync_enabled(params):
                            from ..util.prefill_nosync import upload_metadata
                            tables = upload_metadata(torch.from_numpy(np.stack([base, starts_np, kind])), y.device)
                        else:
                ...
                if groups:
                    batched = self._run_batch_recon(
                        ...
                        scratch, tables, prefill_nosync = _prefill_nosync_enabled(params))
