...
def build_runs_model(local):
    """V2 build_runs == r2_build: active slots sorted by (expert, slot), cut at expert change or every ROWS."""
    act = sorted((e, s) for s, e in enumerate(local) if e >= 0)
    order = [s for _, s in act]
    starts = []
    for t, (e, s) in enumerate(act):
        first = sum(1 for u in local if 0 <= u < e)
        if t == 0 or act[t - 1][0] != e or (t - first) % ROWS == 0:
            starts.append(t)
    return order, starts + [len(act)]
...
def r2_checks():
    ...
    ok = True
    rng = random.Random(1)
    # run table: partition, run size, one expert per run, order
    for trial in range(2000):
        bsz, topk = rng.randint(2, 16), 10
        pool = rng.randint(10, 512)
        local = []
        for r in range(bsz):
            picks = rng.sample(range(pool), topk)
            local += [e if rng.random() > 0.1 else -1 for e in picks]
        order, starts = build_runs_model(local)
        seen = []
        for r in range(len(starts) - 1):
            run = order[starts[r]:starts[r + 1]]
            ok &= 1 <= len(run) <= ROWS and len({local[s] for s in run}) == 1
            seen += run
        ok &= sorted(seen) == [s for s, e in enumerate(local) if e >= 0]
        nact = [sum(1 for k in range(topk) if local[r * topk + k] >= 0) for r in range(bsz)]
        ok &= all(0 <= n <= topk for n in nact)
    # kernel A staging: x copies (16 B, zero-filled past H), rotation tasks, suh preload
    WKs = 16
    for Hi, H in ((2560, 2560), (2560, 2500), (1024, 1000), (4096, 4096)):
        chunks, cpr = Hi // 128, Hi * 2 // 16
        for nrows in range(1, ROWS + 1):
            got = {}
            for j in range(nrows * cpr):
                r, c = j // cpr, j % cpr
                valid = min(max(H - c * 8, 0), 8) * 2
                got[(r, c)] = valid
                ok &= valid == max(0, min(16, (H - c * 8) * 2))
            ok &= len(got) == nrows * cpr
            done = set()
            for w in range(WKs):
                for c in range(w, chunks, WKs):
                    src = "su0" if c == w else "su1" if c == w + WKs else "load"
                    ok &= (src != "load") or chunks > 2 * WKs
                    for r in range(nrows):
                        ok &= (r, c) not in done
                        done.add((r, c))
            ok &= len(done) == nrows * chunks
    # kernel A passes: rows_fit per K at Hi = 2560 (static smem from ptxas: A 1328 B, B 3712 B), 2 CTAs/SM
    V3_RED, SM_BYTES, RESERVED = 16384, 102400, 1024
    fits = {}
    for K in (2, 3, 4):
        ring = 16 * 4 * (2 * 8 * K) * 4
        rows_fit = 0
        for rows in range(ROWS, 0, -1):
            region = max(rows * (2560 * 2 + 16), V3_RED)
            if 2 * (region + ring + 1328 + RESERVED) <= SM_BYTES:
                rows_fit = rows
                break
        fits[K] = rows_fit
        ok &= rows_fit >= 1 and region + ring <= 48 * 1024
        for nrows in range(1, ROWS + 1):
            parts = [(off, min(rows_fit, nrows - off)) for off in range(0, nrows, rows_fit)]
            ok &= sum(n for _, n in parts) == nrows
        b = V3_RED + ring + ROWS * (640 * 2 + 16)
        ok &= 2 * (b + 3712 + RESERVED) <= SM_BYTES and b <= 48 * 1024
    # staged fragment reads: word = r0 * stride + (lane & 3) + 8 ks (+4); stride = width / 2 + 4 words
    for width in (2560, 640):
        stride = width // 2 + 4
        for ks in range(0, width // 16):
            for plus in (0, 4):
                banks = [((l >> 2) * stride + (l & 3) + 8 * ks + plus) % 32 for l in range(32)]
                ok &= len(set(banks)) == 32
    print(f"  r2: run table 2000 routings, staging coverage, rows_fit {fits} (K: rows), bank-conflict-free "
          f"fragment reads: {'PASS' if ok else 'FAIL'}")
    return ok
