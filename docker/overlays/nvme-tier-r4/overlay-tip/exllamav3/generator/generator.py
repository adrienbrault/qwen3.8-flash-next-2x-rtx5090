...
# Recurrent checkpoints at conversation tips (cache/recurrent_tip.py). Unset = no TipStash, every hook below is a
# `self.tip_stash is not None` test that is False.
_RECURRENT_TIP_STASH = os.environ.get("EXL3_RECURRENT_TIP_STASH", "0") == "1"
...
class Generator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tip_stash = None
        if _RECURRENT_TIP_STASH and self.recurrent_cache is not None:
            from ..cache.recurrent_tip import TipStash
            self.tip_stash = TipStash.install(self)
    ...
    def recurrent_checkpoint(self):
        ...
        if self.tip_stash is not None:
            self.tip_stash.prune(self.active_jobs)
    ...
    def iterate_gen(self, results: list, draft_tokens: torch.Tensor | None = None):
        ...
        if draft_tokens is None and batch_logits.shape[1] == 1:
            ...
            for job, token_logits, (next_token, next_k_tokens, next_k_probs, next_prob) in launched:
                ...
                if self.tip_stash is not None and job.sequences[0].kv_position % PAGE_SIZE == 0:
                    self.tip_stash.on_token(job)
        ...
        # must advance between positions.
            ...
            for idx, (job, a, b) in enumerate(zip(self.active_jobs, logit_mapping[:-1], logit_mapping[1:])):
                ...
                for i in range(batch_logits.shape[1]):
                    ...
                    # Tip snapshot: the state at a page boundary reached inside this verification window is read
                    # from the history buffer here, stream-ordered before reject_remainder() rewinds it
                    if self.tip_stash is not None and job.sequences[0].kv_position % PAGE_SIZE == 0:
                        self.tip_stash.on_token(job)
        ...
        for job in completed_jobs + requeuing_jobs:
            ...
            if self.tip_stash is not None:
                self.tip_stash.on_finish(job, results)
            job.deallocate_pages()
            self.active_jobs.remove(job)
    ...
    def iterate_start_jobs(self, results: list):
        ...
        if (self.pagetable.num_unreferenced_pages() and
            ...
            for job in self.pending_jobs.copy():
                ...
                if self.tip_stash is not None:
                    self.tip_stash.before_allocate(job)
                try:
                    job.allocate_pages()
                except Exception as e:
                    self.reap_failed_job(job, e, results)
                    continue
                if self.tip_stash is not None:
                    self.tip_stash.on_allocated(job)
                current_max_batch += len(job.sequences)
