...
class RecurrentCache(OrderedDict):
    def __init__(
        ...
        self.pagetable = None
        # Optionally set by the Generator (EXL3_NVME_TIER): persistent NVMe tier for checkpoints and their chains
        self.disk_tier = None
        self.metrics = {
    ...
    def put(self, key, state):
        ...
        if key in self:
            ...
            if self.disk_tier is not None:
                # A stash must be a snapshot: CPU-resident state (the PLE id context) comes back as a live view
                self.disk_tier.own_stash(state, stashed_state)
            ...
            while self.update_total_size() + state_size > self.max_size:
                ...
                if popped_key is not None:
                    ...
                    if self.disk_tier is not None:
                        self.on_evict(popped_key, popped)
            ...
            if self.disk_tier is not None:
                # Non-blocking hand-off: admission and a queue put; the writer thread serializes and writes
                self.disk_tier.persist_checkpoint(key, stashed_state)


    def on_evict(self, key, stashed_state):
        """
        Eviction seam (also called by the tip-aware policy in recurrent_tip.py, whose evictions bypass the loop in
        put()): a checkpoint that is leaving RAM gets persisted if the NVMe tier does not hold it yet.
        """
        if self.disk_tier is not None:
            self.disk_tier.on_checkpoint_evicted(key, stashed_state)
