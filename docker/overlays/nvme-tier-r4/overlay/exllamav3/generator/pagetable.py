...
class Sequence:
    ...
    def allocate_pages(
        ...
        if recurrent_cache is not None:
            ...
            # NVMe tier: a deeper checkpoint that exists only on disk is loaded into the RAM cache first
            if pagetable.disk_tier is not None:
                restore_limit = pagetable.disk_tier.prepare_resume(
                    page_hashes, recurrent_cache, pagetable, recurrent_pages, restore_limit
                )
...
class PageTable:
    ...
    def __init__(
        ...
        # Optional persistent NVMe tier (DiskPageCache), set by the Generator when EXL3_NVME_TIER is set
        self.disk_tier = None
    ...
    def evict(self, page: CachePage, protect: set | None = None):
        ...
        if page.kv_position == PAGE_SIZE and is_content_hash(page.phash):
            ...
            if self.disk_tier is not None:
                self.disk_tier.store(page, self.access_serial, protect)
    ...
    def allocate_pages(
        ...
        # NVMe tier: bounded parallel reads of the pages to restore, started before the allocation walk
        disk_batch = None
        disk_ops = []
        if self.disk_tier is not None and restore_limit != 0:
            disk_batch = self.disk_tier.begin_restore(
                page_hashes if restore_limit is None else page_hashes[:restore_limit], self
            )
        ...
        for lp, h in enumerate(page_hashes):
            ...
            # If possible, reuse an unreferenced page with matching hash
                ...
                # simply falls through to normal prefill.
                    ...
                    if (
                    ...
                    elif disk_batch is not None and h in disk_batch:
                        entry = disk_batch.take(h, op.page_index)
                        if entry is not None:
                            ...
                            # A page that cannot be restored aborts the whole disk restore of this allocation
                            self.disk_tier.end_restore(disk_batch)
                            disk_batch = None
                            self.disk_tier.abort_restore(disk_ops)
    ...
    def is_resumable(self, phash: bytes) -> bool:
        ...
        while True:
            ...
            elif self.disk_tier is not None and h in self.disk_tier:
                prev = self.disk_tier.prev_of(h)
            else:
                return False
            if prev is None:
                return True
    ...
    def chain_to_root(self, phash: bytes) -> list[bytes] | None:
        """
        Root-first page-hash chain ending at phash, each link complete in VRAM, the CPU tier or the NVMe tier;
        None if the chain is broken. Used by the NVMe tier to admit a checkpoint's chain.
        """
        chain = []
        seen = set()
        h = phash
        while h is not None:
            ...
            if page is not None:
                ...
                h = self.cpu_tier.entries[h]["prev_hash"]
            elif self.disk_tier is not None and h in self.disk_tier:
                h = self.disk_tier.prev_of(h)
            else:
                return None
        chain.reverse()
        return chain
    ...
    def audit_recurrent_sync(self, recurrent_cache) -> dict:
        ...
        def prev_of(h):
            ...
            if self.disk_tier is not None and h in self.disk_tier:
                return True, self.disk_tier.prev_of(h)
            return False, None
