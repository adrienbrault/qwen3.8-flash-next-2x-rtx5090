...
# Round-1 pruned embedding mirror for the device draft chain (default off; needs EXL3_MTP_DEVICE_DRAFT=1,
...
class Generator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # EXL3_EMBED_GPU_PRUNED: build the pruned mirror now, so boot-time free VRAM includes it and an
        # allocation failure surfaces at load rather than on the first request
        if _EMBED_GPU_PRUNED:
            self._pruned_embed = self._prepare_pruned_embed()
    ...
    def iterate(self) -> list[dict]:
        ...
        for job in list(self.active_jobs):
            try:
                if os.environ.get("EXL3_LS_PREFILL_PIPELINE", "0") == "1":
                    from .prefill_pipeline import prefill_job
                    if prefill_job(job, results):
                        continue
                job.prefill(results)
    ...
    def _prepare_pruned_embed(self):
        """
        EXL3_EMBED_GPU_PRUNED=1: (keep_ids, device) when the MTP device chain can run on the pruned mirror,
        else None (the chain then stays on the host path; the full mirror is never built in this mode).
        The mirror lives on the lm_head's device: pruned-head argmax IDs are produced there
        (sample_from_state moves the state to the head with prepare_for_device), so every lookup after the
        first chain position is a same-device gather.
        """
        def off(why):
            logger.warning(f"EXL3_EMBED_GPU_PRUNED=1 inactive: {why}; MTP drafting stays on the host path")
            return None
        if not _MTP_DEVICE_DRAFT:
            return off("EXL3_MTP_DEVICE_DRAFT is not 1")
        if not self.mtp_draft:
            return off("no MTP draft model")
        if self.draft_calibrator is not None:
            return off("dynamic draft (calibrator) disables the device chain")
        te_ref = getattr(self.draft_model, "target_embed", None)
        target_embed = te_ref() if te_ref is not None else None
        keep_fn = getattr(self.draft_model, "draft_head_keep_ids", None)
        keep_ids = keep_fn() if keep_fn is not None else None
        if target_embed is None or keep_ids is None:
            return off("draft head is not pruned (EXL3_MTP_HEAD_N unset or not applicable)")
        lm_head = self.model.modules[self.model.logit_layer_idx]
        if lm_head.device is None or torch.device(lm_head.device).type != "cuda":
            return off("lm_head is not on a CUDA device")
        device = torch.device(lm_head.device)
        pm = target_embed.prepare_pruned_mirror(device, keep_ids)
        if pm is None:
            return off("EXL3_EMBED_GPU is not 1, the embedding is not on the host, or the mirror exceeds EXL3_EMBED_GPU_MAX_MB")
        logger.warning(
            f"EXL3_EMBED_GPU_PRUNED=1: embedding mirror {pm.n} of {pm.num_rows} rows "
            f"({pm.nbytes() / 1024**2:.1f} MiB, {'prefix' if pm.prefix else 'remapped'}) on {device}"
        )
        return keep_ids, device
    ...
    def iterate_draftmodel_mtp_gen(self, results: list):
        ...
        if dev_draft:
            ...
            # Pruned mirror: upload to the head/mirror device; keep the host copy, since the verified token
            # may lie outside the mirrored rows and that is decided on the host (no device sync)
            host_ids0 = batch_ids
            batch_ids = batch_ids.to(
                temp_hidden.device if pruned_embed is None else pruned_embed[1],
        ...
        for idx in range(window):
            ...
            if dev_draft and pruned_embed is not None:
                params["embed_pruned"] = True
                if idx == 0:
                    params["embed_pruned_host_ids"] = host_ids0
    ...
    def iterate_gen(self, results: list, draft_tokens: torch.Tensor | None = None):
        ...
        # must advance between positions.
            ...
            # EOS handling, cache rollback and MTP state updates remain in the serial loop below.
            # Ineligible jobs take the historical serial path.
            batched_verify_tokens = None
            decoded_jobs = [
                job for job, a, b in zip(self.active_jobs, logit_mapping[:-1], logit_mapping[1:])
                if a != b
            ]
            ...
            if batch_key is not None:
                # Serial receive_logits draws once before every consumed sample. Draw the first
                # seed for every job now; later positions advance immediately before consumption.
                # Pure greedy ignores the value, but preserving RNG state keeps fallback/requeue
                # behaviour observationally identical.
                first_seeds = [draw_sampling_seed(job) for job in decoded_jobs]
                sampled = decoded_jobs[0].sampler.forward(
                    batch_logits,
                    None,
                    first_seeds[0],
                    self.tokenizer,
                ...
                batched_verify_tokens = self._staging(
                    "mtp_verify_tokens",
                    sampled.shape[0],
                    sampled.shape[1],
                    torch.long,
                )
                batched_verify_tokens.copy_(sampled, non_blocking = True)
                stream = torch.cuda.current_stream(sampled.device)
                device_key = sampled.device.index
                ready = self.verify_readback_events.get(device_key)
                if ready is None:
                    ready = torch.cuda.Event()
                    self.verify_readback_events[device_key] = ready
