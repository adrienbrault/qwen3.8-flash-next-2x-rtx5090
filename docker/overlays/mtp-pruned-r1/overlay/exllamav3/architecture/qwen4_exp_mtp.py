...
class Qwen4ExpMTPModel(Model):
    ...
    def draft_head_keep_ids(self):
        """
        Vocab IDs the draft chain can emit: the pruned head's columns [0, n_cols) as a cached host LongTensor,
        or None when sample_from_state uses the full head. Same predicate as sample_from_state (_MTP_HEAD_N
        set and _pruned_head() available); the head's device is the lm_head's, where prepare_for_device puts
        the state before the GEMV. Only called by the EXL3_EMBED_GPU_PRUNED=1 path.
        """
        cached = getattr(self, "_draft_head_keep_ids", None)
        if cached is not None:
            return cached if cached is not False else None
        keep = None
        am = self.attached_model() if self.attached_model is not None else None
        if am is not None and _MTP_HEAD_N is not None and _MTP_HEAD_N > 0:
            lm = am.modules[am.logit_layer_idx]
            if getattr(lm, "device", None) is not None:
                pruned = self._pruned_head(lm, torch.device(lm.device))
                if pruned is not None:
                    keep = torch.arange(pruned[2], dtype = torch.long)
        self._draft_head_keep_ids = keep if keep is not None else False
        return keep
