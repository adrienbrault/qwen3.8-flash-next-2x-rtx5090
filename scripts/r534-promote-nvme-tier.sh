#!/usr/bin/env bash
# R534 — promote the NVMe prefix tier (nvme-tier r4, patches/exllamav3/nvme-tier/r4; user 2026-09-19: persistent NVMe tier
# on /srv/qwen5090/fast, no host-RAM tier). Image tabbyapi:nvme-tier-r4 = live image (mtp-pruned-r1-tc1-plefix) + tier
# overlay; R532 passed every tier step on the served chain (restore full coverage, output = warm = cold; drain decode within
# -4 %; cap churn) and the 8-boot idle-decode A/B. Launcher = live + launcher-nvme-tier.patch (fuzz 0) + IMG + a daily
# default: NVME_TIER=/srv/qwen5090/fast/exl3-nvme-daily, NVME_TIER_GB=64 (≈ 2.7M tokens at ~24 KB/token), ONLY when IMG is
# the tier image and NVME_TIER is unset — experiment boots reusing the launcher with other images never open the daily's
# dir (the tier removes stale namespaces at open). Gates: R530's G1-G5 with the tier ON, plus GT: fill 30k, drain, docker
# rm -f + relaunch, the same prompt must hit >= 29,952 cached tokens from disk with output = the pre-restart warm output.
# PASS -> launch-flashnext.sh := candidate (old live kept as launch-flashnext.sh.pre-r534).
# RUN (queued): popped by r515-queue-chain from flashnext-queue.txt.
set -uo pipefail
export HOME=$HOME PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r534-promote-nvme-tier; mkdir -p "$R"
...
CAND=/srv/qwen5090/launch-flashnext-r534.sh
SRC=/srv/qwen5090/overlay-src/nvme-tier-r4
TDIR=/srv/qwen5090/fast/exl3-nvme-daily; TGB=64
SALT=$(( $(date +%s) % 100000 ))
...
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r534] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R534 $1 ==="; }
...
grep -q 'NVME_TIER' "$LIVE" && { log "ABORT: live launcher already has the tier"; exit 3; }
NIMG=tabbyapi:nvme-tier-r4
LENV=$(sed -n 's/^EXTRA_ENV=\${EXTRA_ENV:-\(.*\)}$/\1/p' "$LIVE")
C1=e7fb377c987d685c; C30=4a255910dee2d9c5
case " $LENV " in *" EXL3_EMBED_GPU_PRUNED=1 "*) REFS="221.8 194.5 508.4 484.4";; *) log "ABORT: live env lacks the pruned draft"; exit 3;; esac
read -r size avail < <(df -B1 --output=size,avail /srv/qwen5090/fast | tail -1)
[ $(( avail - TGB * 1024**3 - size * 15 / 100 )) -gt 0 ] || { log "ABORT: /srv/qwen5090/fast cannot hold a ${TGB} GiB tier above 15 % free ($((avail>>30)) GiB free)"; exit 3; }
log "building $NIMG on $LIMG (before the lock)"
...
grep -aq "stack-r4-e3r2+mtp-pruned-r1: 5 files installed" "$R/build.log" && grep -aq "nvme-tier smoke OK" "$R/build.log" || { log "ABORT: wrong variant or smoke missing"; exit 3; }
cp "$LIVE" "$CAND.new" && patch --fuzz=0 "$CAND.new" < "$SRC/launcher-nvme-tier.patch" > "$R/launcher-patch.log" 2>&1 || { log "ABORT: launcher patch"; exit 3; }
python3 - "$CAND.new" "$LIMG" "$NIMG" "$TDIR" "$TGB" <<'PY' || { log "ABORT: candidate edit"; exit 3; }
import sys; p,limg,nimg,tdir,tgb=sys.argv[1:6]; s=open(p).read()
old="IMG=${IMG:-%s}"%limg; assert s.count(old)==1; s=s.replace(old,"IMG=${IMG:-%s}"%nimg)
old="NVME_TIER=${NVME_TIER:-}\n"; assert s.count(old)==1
s=s.replace(old,"""# Daily default (R534): the tier below when IMG is the served tier image and NVME_TIER is unset. Any other image, or an
...
if [ -z "${NVME_TIER+x}" ]; then
  if [ "$IMG" = %s ]; then NVME_TIER=%s; NVME_TIER_GB=${NVME_TIER_GB:-%s}; else NVME_TIER=; fi
fi
"""%(nimg,tdir,tgb))
s=s.replace("# NVMe prefix tier (nvme-tier-r3, opt-in)","# NVMe prefix tier (nvme-tier-r4, opt-in)")
open(p,"w").write(s)
PY
chmod 755 "$CAND.new" && mv "$CAND.new" "$CAND"
diff "$LIVE" "$CAND" | tee "$R/candidate.diff" | head -40 >> "$R/audit.log"
export GPU_QUEUE_NAME=r534-promote-nvme-tier
...
tl=$(sudo docker logs flashnext 2>&1 | grep -a -- ' -- nvme tier: ON' | tail -1 | cut -c1-220)
case "$tl" in *"cap $TGB.0 GiB"*) ;; *) log "G1 FAIL: tier line '$tl'"; finish ABORTED; exit 3;; esac
sudo docker inspect -f '{{range .Mounts}}{{.Source}} {{end}}' flashnext | grep -q "$TDIR" || { log "G1 FAIL: tier dir not mounted"; finish ABORTED; exit 3; }
sudo docker logs flashnext 2>&1 | grep -aq "EXL3_EMBED_GPU_PRUNED=1 inactive" && { log "G1 FAIL: pruned mirror inactive"; finish ABORTED; exit 3; }
log "G1 PASS: $got; image $img; tier: $tl; VRAM free MiB $(vram)"
...
log "=== GT: crash-restart restore on the daily configuration ==="
T=$SRC/tests/gpu_nvme_ab.py
timeout 600 python3 "$T" fill --out "$R" --sizes 30000 2>&1 | tail -3 | cut -c1-220 | tee -a "$R/audit.log"
timeout 300 python3 "$T" wait-drained --out "$R" --container flashnext 2>&1 | tail -2 | cut -c1-240 | tee -a "$R/audit.log"
sudo docker logs flashnext > "$R/docker-pre-restart.log" 2>&1; sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$CAND" > "$R/boot-restart.log" 2>&1 || { log "GT FAIL: relaunch"; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
sudo docker logs flashnext 2>&1 | grep -a "open scan" | tail -1 | cut -c1-200 | tee -a "$R/audit.log"
timeout 600 python3 "$T" verify --out "$R" 2>&1 | tail -4 | cut -c1-240 | tee "$R/verify.txt" | tee -a "$R/audit.log"
grep -aq "VERIFY PASS" "$R/verify.txt" && log "GT PASS" || { log "GT FAIL"; finish ABORTED; exit 3; }
log "=== G2: agentic-edit ==="
python3 /srv/qwen5090/probes/agentic-edit.py --url "$API" --model "$NEWM" --tag NT --conc 1 4 --modes greedy sampled --out "$R/agentic-edit.jsonl" 2>&1 | tee "$R/agentic-edit.log" | grep -a "agentic-edit" | tee -a "$R/audit.log"
...
python3 /srv/qwen5090/probes/tooleval_summary.py "$R/tooleval.json" NT 2>&1 | tee "$R/tooleval-summary.txt" | tee -a "$R/audit.log"
...
log "GSM8K n=500 flexible: ${g:-none} (R530 0.974)"
...
log "=== PROMOTE: launch-flashnext.sh := launch-flashnext-r534.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r534
cp "$CAND" "$LIVE.new" && chmod 755 "$LIVE.new" && mv "$LIVE.new" "$LIVE"
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r534 "$LIVE"; finish ABORTED; exit 3; }
PROMOTED=1
sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || { log "LIVE LAUNCH FAILED — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r534 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
a2=$(greedy live); log "daily now: $(served_id) $(cfgline); c1 fingerprint $a2 (promotion-boot $a); VRAM free MiB $(vram)"
[ "$(served_id)" = "$NEWM" ] || { log "daily did not come up — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r534 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
finish PROMOTED
