#!/usr/bin/env bash
# R485 — fine pool frontier: chunk 2048/1024/512 x six splits x 8,192-token pool steps (boot only).
#
...
#
# R483 RESULT (coarse): [30, 31] boots 393,216 at chunk 2048 (cuda:0 then has 2,075 MiB free: a whole layer moved to cuda:1)
# and 409,600 at chunk 1024; chunk 512 did not reach 425,984. Boots take ~16 s from page cache, so R485 maps the frontier
# finely: for each chunk and split, the largest pool from 442,368 down in 8,192 steps (stop at the first that serves).
# No measurement here: the chosen config goes through R483-style measurement + promotion gates afterwards.
#
# (R483 header follows)
...
#
# RUN: sudo systemd-run --unit=r485-pool-frontier --collect -p RuntimeMaxSec=43200 -E HOME=$HOME \
#        /bin/bash /srv/qwen5090/r485-pool-frontier.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r485-pool-frontier; mkdir -p "$R"
...
SERVED=360448
BOOTED=0
log(){ echo "$(date -Is) [r485] $*" | tee -a "$R/audit.log"; }
...
finish(){ if [ "$BOOTED" = 1 ]; then log "restoring the daily (unchanged live launcher)"
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R485 $1 ==="; }
...
export GPU_QUEUE_NAME=r485-pool-frontier
...
[ "$(served_id)" = "$MODEL" ] || { log "ABORT: the daily is not serving at entry"; finish ABORTED; exit 3; }
POOLS="442368 434176 425984 417792 409600 401408 393216 385024"
for chunk in 2048 1024 512; do
  for split in "30.5, 31" "30.75, 31" "31, 31" "30.25, 31.25" "30.5, 31.25" "31, 31.25"; do
    best=0
    for c in $POOLS; do
      try_boot "c$chunk" "$split" "$chunk" "$c"; rc=$?
      [ $rc = 2 ] && { finish ABORTED; exit 3; }
      [ $rc = 0 ] && { best=$c; break; }
    done
    log "FRONTIER chunk $chunk split [$split]: $best"; echo "$chunk|$split|$best" >> "$R/frontier.tsv"
  done
done
finish DONE
