#!/usr/bin/env bash
# R486 — which knob flips the c1 greedy fingerprint: split [30, 31] or chunk 1024 (both at the served 360,448).
#
...
#
# R483 FINDING: the winner ([30, 31], chunk 1024, 409,600) gives c1 greedy 7812e069df67f255 (served 1474eee2f5945248), diverging
# at ~160 tokens on a near-tie word, while its 30k greedy is canonical. Arms at 360,448: CTL [30, 30]/2048 (determinism: two
# greedy calls), S [30, 31]/2048, K [30, 30]/1024, SK [30, 31]/1024. Each: two c1 greedy calls + 30k greedy + boot-log layer map + code c1 x2 + cold prefill 30k/120k (R483 D lost ~20 % code
# c1 and half the prefill; which knob costs what).
#
# (R483 header follows)
...
#
# RUN: sudo systemd-run --unit=r486-fp-diag --collect -p RuntimeMaxSec=43200 -E HOME=$HOME \
#        /bin/bash /srv/qwen5090/r486-fp-diag.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r486-fp-diag; mkdir -p "$R"
...
SERVED=360448
BOOTED=0
log(){ echo "$(date -Is) [r486] $*" | tee -a "$R/audit.log"; }
...
finish(){ if [ "$BOOTED" = 1 ]; then log "restoring the daily (unchanged live launcher)"
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R486 $1 ==="; }
...
export GPU_QUEUE_NAME=r486-fp-diag
...
[ "$(served_id)" = "$MODEL" ] || { log "ABORT: the daily is not serving at entry"; finish ABORTED; exit 3; }
for arm in "CTL|30, 30|2048" "S|30, 31|2048" "K|30, 30|1024" "SK|30, 31|1024"; do
  IFS='|' read -r tag split chunk <<< "$arm"
  try_boot "$tag" "$split" "$chunk" 360448; rc=$?
  [ $rc = 2 ] && { finish ABORTED; exit 3; }
  [ $rc = 0 ] || continue
  sudo docker logs flashnext 2>&1 | grep -aiE "cuda:[01]|split|layer" | head -40 > "$R/layers-$tag.txt"
  greedy "$tag-a"; greedy "$tag-b"; greedy30k "$tag"
  python3 /srv/qwen5090/probes/fn_bench.py --url "$API" --model "$MODEL" --tag "$tag-code" --kind code --tokens 2048 --conc 1 --runs 2 \
    --out "$R/records-$tag.jsonl" 2>&1 | grep -E "^  c=|FAILED|Traceback|Error" | sed "s/^/[$tag code]/" | cut -c1-260 | tee -a "$R/audit.log"
  prefill "$tag"
  log "[$tag] prefill-pipeline lines: $(sudo docker logs flashnext 2>&1 | grep -aiE 'pipeline' | tail -2 | tr '\n' ' ' | cut -c1-240)"
done
finish DONE
