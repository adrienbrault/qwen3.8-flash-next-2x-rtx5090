#!/usr/bin/env bash
# R498 — the served 3.05bpw with r0b0tlab's 4-bit MTP head grafted in (qwen3.8-flash-next-exl3-3.05bpw-mtp4, built by
# r498-mtp4-graft-build.sh) vs the served 3-bit head. WHY (2026-09-18 survey; user: "isn't there more things to try"): halogen
# re-quantized its MTP head at 8 bits (prose acceptance 51 -> 59 %, +4 % prose decode). Verification is exact and the verify
# shape is fixed by the depth policy, so greedy output must stay canonical on every arm; only acceptance (decode speed) moves.
# Launcher-r498 = the live launcher-r491 + DYN (off) + CKPT_NAME; image = the daily's (decode-kernels-r2, overlap on).
# ARMS: BASE / GRAFT / BASE2 / GRAFT2 — fingerprints (must be canonical), fn_bench code + prose c1/c4 2,048 x 2, multiprompt.
# RUN: sudo systemd-run --unit=r498-mtp4-graft --collect -p RuntimeMaxSec=86400 -E HOME=$HOME /bin/bash /srv/qwen5090/r498-mtp4-graft.sh
set -uo pipefail
...
BOOTED=0
log(){ echo "$(date -Is) [r498] $*" | tee -a "$R/audit.log"; }
...
finish(){ sudo docker rm -f r498-test >/dev/null 2>&1
  if [ "$BOOTED" = 1 ]; then log "restoring the daily (unchanged live launcher)"
    sudo docker rm -f flashnext >/dev/null 2>&1
    "${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || log "RESTORE LAUNCHER FAILED"
    MODEL=qwen3.8-flash-next-exl3-3.05bpw; for i in $(seq 240); do [ "$(served_id)" = "$MODEL" ] && break; sleep 2; done
    log "daily: $(served_id) $(cfgline) VRAM free $(vram)"; fi
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R498 $1 ==="; }
...
export GPU_QUEUE_NAME=r498-mtp4-graft
...
boot(){ local tag=$1 ck=$2 cache=${3:-360448} i st lp got; MODEL=$ck
  log "boot $tag: checkpoint $ck"
  sudo docker rm -f flashnext >/dev/null 2>&1; for i in $(seq 30); do [ "$(served_id)" = "<no answer>" ] && break; sleep 2; done
  ( "${CLEAN_ENV[@]}" IMG="$IMG" CACHE=$cache CKPT_NAME=$ck EXTRA_ENV="$BASE_ENV" bash "$CAND" >> "$R/boot-$tag-$cache.log" 2>&1 ) & lp=$!
  for i in $(seq 360); do
...
for arm in "BASE|qwen3.8-flash-next-exl3-3.05bpw" "GRAFT|qwen3.8-flash-next-exl3-3.05bpw-mtp4" "BASE2|qwen3.8-flash-next-exl3-3.05bpw" "GRAFT2|qwen3.8-flash-next-exl3-3.05bpw-mtp4"; do
  IFS='|' read -r tag ck <<< "$arm"
  # the 4-bit MTP layer is larger than the 3-bit one: if 360,448 no longer fits the [30, 30] split, fall back one step
  POOLX=360448; boot "$tag" "$ck" 360448; rc=$?
  [ $rc = 2 ] && { finish ABORTED; exit 3; }
  if [ $rc = 1 ]; then [ "${tag#GRAFT}" != "$tag" ] || { finish ABORTED; exit 3; }; POOLX=344064; boot "$tag" "$ck" 344064 || { finish ABORTED; exit 3; }; fi
  sudo docker logs flashnext 2>&1 | grep -aiE "overrid" | head -3 | cut -c1-200 | tee -a "$R/audit.log"
  if ! fp "$tag"; then
    if [ "$POOLX" = 360448 ]; then log "[$tag] FINGERPRINT NOT CANONICAL at the served pool"; finish ABORTED; exit 3; fi
    log "[$tag] fingerprint differs at pool $POOLX (placement change; recorded)"; fi
  speed "$tag"
done
MODEL=qwen3.8-flash-next-exl3-3.05bpw
finish DONE
