#!/usr/bin/env bash
# R487 — measure split [30, 31] / chunk 2048 / 393,216 (+9 % pool, the chunk-2048 frontier) against the served daily.
#
...
#
# R485/R486: chunk 1024/512 disable the prefill pipeline (prefill halves); the split alone is byte-identical and as fast at
# 360,448; the chunk-2048 frontier is 393,216 under every split tried. At 393,216 a layer moves to cuda:1 (VRAM free 2,075 /
# 721 MiB) — R483 D (409,600) lost ~20 % code c1 and flipped the c1 fingerprint with a moved layer, so this candidate needs
# the full measurement: ctl (served, decode + prefill, no reboot) then C with fingerprints, decode, prefill, 4 x 60k, needles,
# GSM8K. Promotion is a separate gated unit.
#
# (R483 header follows)
...
#
# RUN: sudo systemd-run --unit=r487-pool-393k --collect -p RuntimeMaxSec=43200 -E HOME=$HOME \
#        /bin/bash /srv/qwen5090/r487-pool-393k.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r487-pool-393k; mkdir -p "$R"
...
SERVED=360448
BOOTED=0
log(){ echo "$(date -Is) [r487] $*" | tee -a "$R/audit.log"; }
...
finish(){ if [ "$BOOTED" = 1 ]; then log "restoring the daily (unchanged live launcher)"
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R487 $1 ==="; }
...
export GPU_QUEUE_NAME=r487-pool-393k
...
try_boot C "30, 31" 2048 393216; rc=$?
[ $rc = 0 ] || { log "ABORT: candidate did not boot (rc $rc)"; finish ABORTED; exit 3; }
sudo docker logs flashnext 2>&1 | grep -aiE "pipeline" | tail -2 | cut -c1-400 >> "$R/audit.log"
measure C
finish DONE
