#!/usr/bin/env bash
# R489 — codex gdn-state r1: GDN recurrent state without the max_history+1 fp32 copies (EXL3_GDN_STATE_REPLAY=1).
#
# WHY (user 2026-09-18: "Keep experimenting and pushing the setup. Decode speed and kv pool size"): the recurrent state is
# [num_slots, max_history + 1, 48, 128, 128] fp32 per GDN layer = ~1.77 GiB at 4 slots / depth 3. Codex r1
# (flan/patches/exllamav3/gdnstate/r1) keeps one base state per slot, caches the per-token recurrence inputs of the verify
# pass in two ping-pong banks and applies the accepted tokens at the start of the next GDN launch (apply-then-verify), with
# the same per-token float operations, so the output must stay bit-identical. Projected reclaim 1.34 GB (~639 MiB/card),
# ~+83k tokens. Default 0 = served path (no replay tensors, unchanged kernel).
#
# SEQUENCE (every gate aborts the unit on failure; the daily is restored at the end):
#   build   tabbyapi:gdn-state-r1 from flan/patches/exllamav3/gdnstate/r1/Dockerfile.box (JIT rebuild of the extension)
#   gate 1  kernel identity: tests/test_gdn_replay_kernel.py on cuda:0 — must print "PASS: 2485 torch.equal checks"
#   gate 2  real checkpoint token ids OFF vs ON: model_decode_ids.py single / lifecycle / checkpoint + compare_model_ids.py
#   gate 3  serving at the SERVED pool 360,448 (placement unchanged): OFF and ON fingerprints must both be canonical
#           (1474eee2f5945248 / 4a255910dee2d9c5) — a bigger pool can move a layer to cuda:1, which flips greedy output by
#           itself (R483/R487), so identity is judged here
#   ladder  ON at 458,752 / 425,984 / 409,600 / 393,216 (split [30, 30], chunk 2048)
#   measure the ON winner and OFF at 360,448: fn_bench code + prose c1/c4 2,048 × 2, probes/multiprompt.py (12 prompts per
#           kind, sampled), cold prefill 30k / 120k, 4 × 60k concurrent, needle 131k / 240k, GSM8K n=200
#
# RUN: sudo systemd-run --unit=r489-gdn-state --collect -p RuntimeMaxSec=43200 -E HOME=$HOME \
#        /bin/bash /srv/qwen5090/r489-gdn-state.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r489-gdn-state; mkdir -p "$R"
...
CTX=/srv/qwen5090/build/gdnstate-r1
IMG=tabbyapi:gdn-state-r1
...
BOOTED=0
log(){ echo "$(date -Is) [r489] $*" | tee -a "$R/audit.log"; }
...
finish(){ sudo docker rm -f r489-test >/dev/null 2>&1
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R489 $1 ==="; }
...
export GPU_QUEUE_NAME=r489-gdn-state
...
log "=== gate 1: kernel identity ==="
sudo docker run --rm --name r489-test --gpus all --ipc=host --entrypoint bash "$IMG" \
  -lc 'python3 /opt/gdn-state-r1/tests/test_gdn_replay_kernel.py --device cuda:0' > "$R/gate1-kernel.log" 2>&1
tail -2 "$R/gate1-kernel.log" | tee -a "$R/audit.log"
grep -q "^PASS: 2485 torch.equal checks" "$R/gate1-kernel.log" || { log "GATE 1 FAIL"; finish ABORTED; exit 3; }

log "=== gate 2: real-checkpoint token ids OFF vs ON ==="
mkdir -p "$R/g2"; sudo chmod 777 "$R/g2"
G2OK=1
for sc in "single|--tokens 512" "lifecycle|" "checkpoint|--tokens 300"; do
  IFS='|' read -r scen extra <<< "$sc"
  for MODE in off on; do REPLAY=$([ $MODE = on ] && echo 1 || echo 0)
    EV=(); for kv in $BASE_ENV; do EV+=(-e "$kv"); done
    sudo docker run --rm --name r489-test --gpus all --ipc=host --entrypoint bash -v "$CKPT":/models:ro -v "$R/g2":/results \
      "${EV[@]}" -e EXL3_GDN_STATE_REPLAY=$REPLAY "$IMG" \
      -lc "python3 /opt/gdn-state-r1/tests/model_decode_ids.py --model /models --cache-size 8192 --max-batch-size 4 --max-history 3 --vision $extra --scenario $scen --output /results/model-$scen-$MODE.json" \
      > "$R/g2/model-$scen-$MODE.log" 2>&1 || { log "gate 2 $scen $MODE exited non-zero: $(tail -2 "$R/g2/model-$scen-$MODE.log" | tr '\n' ' ' | cut -c1-240)"; G2OK=0; }
  done
  out=$(python3 "$CTX/tests/compare_model_ids.py" "$R/g2/model-$scen-off.json" "$R/g2/model-$scen-on.json" 2>&1); rc=$?
  log "gate 2 $scen: rc $rc $(echo "$out" | tail -2 | tr '\n' ' ' | cut -c1-240)"; [ $rc = 0 ] || G2OK=0
done
[ $G2OK = 1 ] || { log "GATE 2 FAIL"; finish ABORTED; exit 3; }
...
boot(){ local tag=$1 rp=$2 cache=$3 i st lp got
  ...
  ( "${CLEAN_ENV[@]}" IMG="$IMG" CACHE=$cache EXTRA_ENV="$BASE_ENV EXL3_GDN_STATE_REPLAY=$rp" bash "$CAND" >> "$R/boot-$tag-$cache.log" 2>&1 ) & lp=$!
  for i in $(seq 360); do
...
log "=== gate 3: serving at 360,448, OFF then ON, fingerprints canonical ==="
boot OFF 0 360448 || { finish ABORTED; exit 3; }
fp OFF || { log "GATE 3 FAIL (OFF on the new image is not canonical)"; finish ABORTED; exit 3; }
boot ON360 1 360448 || { finish ABORTED; exit 3; }
fp ON360 || { log "GATE 3 FAIL (replay changes greedy output)"; finish ABORTED; exit 3; }
