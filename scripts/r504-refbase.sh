#!/usr/bin/env bash
# R504 — gate tabbyapi:decode-kernels-r2-refbase (the daily + codex's reference-tree files) before four codex overlays build on it.
# WHY: codex's worktrees carry a `ref/served-src` that is the served code PLUS two earlier default-off experiments (R473 decode
# overlap / batched greedy verify, EXL3_DECODE_OVERLAP; fused V2 mixer, EXL3_DECODE_FUSE). Rounds r4 (R499), r5 (R505), prompt
# lookup (R501) and sched (R502) were written against it: hash-pinned overlays abort on the daily and whole-file overlays import
# generator/draft_overlap.py, absent from the daily. Found 2026-09-18 18:55 CEST before any of them ran (GNU dry-run of r5).
# The refbase image adds the nine differing files (flan/patches/exllamav3/refbase), both flags off. Gate: DAILY / REF / DAILY2 /
# REF2 through launcher-r498 (overlap on), fingerprints canonical on every arm, fn_bench code + prose c1/c4 2,048 x 2,
# multiprompt 12 x kind 2,048 c1/c4. PASS = canonical everywhere and no speed regression beyond the arm-to-arm band.
# RUN: sudo systemd-run --unit=r504-refbase --collect -p RuntimeMaxSec=86400 -E HOME=$HOME /bin/bash /srv/qwen5090/r504-refbase.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r504-refbase; mkdir -p "$R"
...
CTX=/srv/qwen5090/build/refbase
IMG=tabbyapi:decode-kernels-r2-refbase
...
BOOTED=0
log(){ echo "$(date -Is) [r504] $*" | tee -a "$R/audit.log"; }
...
finish(){ sudo docker rm -f r504-test >/dev/null 2>&1
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R504 $1 ==="; }
...
export GPU_QUEUE_NAME=r504-refbase
...
boot(){ local tag=$1 img=$2 cache=360448 i st lp got
  log "boot $tag: image $img"
  sudo docker rm -f flashnext >/dev/null 2>&1; for i in $(seq 30); do [ "$(served_id)" = "<no answer>" ] && break; sleep 2; done
  ( "${CLEAN_ENV[@]}" IMG="$img" CACHE=$cache EXTRA_ENV="$BASE_ENV" bash "$CAND" >> "$R/boot-$tag-$cache.log" 2>&1 ) & lp=$!
  for i in $(seq 360); do
...
for arm in "DAILY|tabbyapi:decode-kernels-r2" "REF|$IMG" "DAILY2|tabbyapi:decode-kernels-r2" "REF2|$IMG"; do
  tag=${arm%%|*}; img=${arm#*|}
  boot "$tag" "$img" || { finish ABORTED; exit 3; }
  fp "$tag" || { log "[$tag] FINGERPRINT NOT CANONICAL"; finish ABORTED; exit 3; }
  speed "$tag"
done
log "R504 PASS on identity (all four arms canonical); read the speed lines for the regression check"
finish DONE
