#!/usr/bin/env bash
# R540 — promote decode-kernels r6 (patches/exllamav3/decode-kernels/r6, r5 rebased onto the served chain): EXL3_GDN_BA_WARP1
# (one-warp GDN B/A GEMV at c1), EXL3_HC_APPLY_WARP1 (one-warp hc_apply tiles), EXL3_GR_STATE_REGRID (V2 state re-grid, int8
# mixer port). Bit-order preserving. R538 (8 boots, served chain): kernel equality 224/224 cases on both cards, fingerprints
# canonical on all 8 boots, code c1 +0.99 % [+0.39, +1.59], prose c1 +1.10 % [+0.74, +1.45], c4 −0.17 % [−0.93, +0.58], VRAM
# unchanged. Candidate = live launcher with the image name replaced in BOTH places (IMG default and the tier-default condition)
# + the three selectors in EXTRA_ENV. The NVMe tier namespace changes (engine identity hashes the installed sources).
# PASS -> launch-flashnext.sh := candidate (old live kept as launch-flashnext.sh.pre-r540).
# RUN (queued): popped by r515-queue-chain from flashnext-queue.txt.
set -uo pipefail
export HOME=$HOME PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r540-promote-r6; mkdir -p "$R"
...
CAND=/srv/qwen5090/launch-flashnext-r540.sh
...
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r540] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R540 $1 ==="; }
...
[ "$LIMG" = tabbyapi:nvme-tier-r4-e3det ] || { log "ABORT: live image $LIMG is not the R535 image"; exit 3; }
NIMG="$LIMG-r6"
LENV=$(sed -n 's/^EXTRA_ENV=\${EXTRA_ENV:-\(.*\)}$/\1/p' "$LIVE")
case " $LENV " in *" EXL3_GDN_BA_WARP1=1 "*) log "ABORT: r6 already live"; exit 3;; *" EXL3_MOE_PREFILL_E3_DET=1 "*) ;; *) log "ABORT: live env has no E3 DET"; exit 3;; esac
...
(cd "$SRC" && sudo docker build -f overlay/Dockerfile.box --build-arg BASE="$LIMG" -t "$NIMG" . ) > "$R/build.log" 2>&1 || { log "ABORT: build failed: $(tail -3 "$R/build.log" | tr '\n' ' ' | cut -c1-240)"; exit 3; }
# ask the image, not the build log: a cached rebuild prints none of the RUN output (R540 try 2)
sudo docker run --rm --entrypoint python3 "$NIMG" -c "import exllamav3.modules.hyperconnections as h; from exllamav3.ext import exllamav3_ext as e; assert h.GatedResidual.STATE_REGRID is False; assert hasattr(e, 'gr_mix_v2_int8_regrid'); assert hasattr(e, 'exl3_moe_prefill_e3_det'); print('r6 selectors default off, kernels present')" >> "$R/build.log" 2>&1 \
  || { log "ABORT: image does not have the r6 kernels with the selectors default off"; exit 3; }
...
assert s.count(limg)==2, s.count(limg)   # IMG default + tier-default condition
s=s.replace(limg,nimg)
m=re.search(r"^EXTRA_ENV=\$\{EXTRA_ENV:-(.*)\}$", s, re.M); assert m
s=s[:m.start(1)]+m.group(1)+" EXL3_GDN_BA_WARP1=1 EXL3_HC_APPLY_WARP1=1 EXL3_GR_STATE_REGRID=1"+s[m.end(1):]
open(dst,"w").write(s)
PY
...
export GPU_QUEUE_NAME=r540-promote-r6
...
for f in EXL3_GDN_BA_WARP1=1 EXL3_HC_APPLY_WARP1=1 EXL3_GR_STATE_REGRID=1 EXL3_MOE_PREFILL_E3_DET=1 EXL3_HC_MIX_V2_INT8=1 EXL3_MOE_PREFILL_E3=1 EXL3_SHARED_EXPERT_OVERLAP=1 EXL3_MTP_HEAD_N=65536; do
  echo "$envs" | grep -qx "$f" || { log "G1 FAIL: container env lacks $f"; finish ABORTED; exit 3; }; done
tl=$(sudo docker logs flashnext 2>&1 | grep -a -- ' -- nvme tier: ON' | tail -1 | cut -c1-200)
[ -n "$tl" ] || { log "G1 FAIL: NVMe tier not ON"; finish ABORTED; exit 3; }
...
# 60k floor 9,000 = 9,200 x 0.98: R533 measured DET at 0.981x there (R535 try 1: 9,136 vs 9,200)
...
log "=== G2: agentic-edit ==="
python3 /srv/qwen5090/probes/agentic-edit.py --url "$API" --model "$NEWM" --tag DT --conc 1 4 --modes greedy sampled --out "$R/agentic-edit.jsonl" 2>&1 | tee "$R/agentic-edit.log" | grep -a "agentic-edit" | tee -a "$R/audit.log"
...
python3 /srv/qwen5090/probes/tooleval_summary.py "$R/tooleval.json" DT 2>&1 | tee "$R/tooleval-summary.txt" | tee -a "$R/audit.log"
...
log "=== PROMOTE: launch-flashnext.sh := launch-flashnext-r540.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r540
cp "$CAND" "$LIVE.new" && chmod 755 "$LIVE.new" && mv "$LIVE.new" "$LIVE"
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r540 "$LIVE"; finish ABORTED; exit 3; }
PROMOTED=1
sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || { log "LIVE LAUNCH FAILED — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r540 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
a2=$(greedy live); log "daily now: $(served_id) $(cfgline); c1 fingerprint $a2 (promotion-boot $a); VRAM free MiB $(vram)"
[ "$(served_id)" = "$NEWM" ] || { log "daily did not come up — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r540 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
finish PROMOTED
