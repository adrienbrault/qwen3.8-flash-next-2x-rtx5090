...
# R482 (2026-09-18, user: cold-expert offload "yes try this"): MOE_SPLIT = routed experts per MoE layer kept on the CPU
# (TabbyAPI cpu_moe_split_experts -> exllamav3 moe_cpu_split). The engine starts with the TAIL N by index and, by default
# (EXL3_MOE_CPU_SWAP=1), swaps the hottest CPU expert with the coldest GPU expert every 128 decode steps, so the CPU set
# converges on the coldest experts for the served traffic. EXL3_MOE_CPU_SPLIT_LAYERS=L (via EXTRA_ENV) splits only the
# first L MoE layers. Mutually exclusive with MOE_OFFLOAD. Default 0 = the served configuration.
MOE_SPLIT=${MOE_SPLIT:-0}
case "$MOE_SPLIT" in [0-9]|[0-9][0-9]|[0-9][0-9][0-9]) ;; *) echo "ABORT: MOE_SPLIT must be an expert count (got $MOE_SPLIT)"; exit 3;; esac
[ "$MOE_SPLIT" != 0 ] && [ "$MOE_OFFLOAD" != 0 ] && { echo "ABORT: MOE_SPLIT and MOE_OFFLOAD are mutually exclusive"; exit 3; }
...
model:
  ...
  cpu_moe_split_experts: $MOE_SPLIT   # set explicitly: a stale nonzero elsewhere would silently change the baseline
  chunk_size: 2048
...
log "starting on 0.0.0.0:$PORT, slots $MAXBS, cache $CACHE @ $CACHE_MODE, moe offload $MOE_OFFLOAD, moe split $MOE_SPLIT, split [$GPU_SPLIT], draft depth $DRAFT, policy '${DRAFT_POLICY:-none}'${DRAFT_DERIVED:+ (derived from DRAFT)}"
