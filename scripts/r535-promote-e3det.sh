#!/usr/bin/env bash
# R535 — promote deterministic E3 prefill (e3-det r1, patches/exllamav3/e3-det/r1): EXL3_MOE_PREFILL_E3_DET=1 replaces the
# atomicAdd accumulation in the E3 grouped MoE prefill with per-assignment slots + a fixed-order reduction, so cold prefill
# is run-to-run and cross-process bit-identical (R531: kernel 6/6 cases, e2e 8k/30k reps identical, atomic diverges by token
# 47-172). R533 (8 boots, served chain): prefill 60k 0.981x [-4.6, +0.9] %, 120k 0.998x [-1.9, +1.5] %, decode c1 +0.3 %,
# fingerprints canonical, VRAM unchanged. Built on the LIVE image (after R534: tabbyapi:nvme-tier-r4; e3-det touches only the
# E3 kernel files + block_sparse_mlp.py, which no served overlay touches). Candidate = live launcher with the tier image name
# replaced in BOTH places (IMG default and the tier-default condition, else the tier would silently turn off) + DET in
# EXTRA_ENV. The NVMe tier namespace changes (engine identity hashes the installed sources): the old namespace is dropped.
# PASS -> launch-flashnext.sh := candidate (old live kept as launch-flashnext.sh.pre-r535).
# RUN (queued): popped by r515-queue-chain from flashnext-queue.txt.
set -uo pipefail
export HOME=$HOME PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r535-promote-e3det; mkdir -p "$R"
...
CAND=/srv/qwen5090/launch-flashnext-r535.sh
...
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r535] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R535 $1 ==="; }
...
[ "$LIMG" = tabbyapi:nvme-tier-r4 ] || { log "ABORT: live image $LIMG is not the R534 tier image"; exit 3; }
NIMG="$LIMG-e3det"
LENV=$(sed -n 's/^EXTRA_ENV=\${EXTRA_ENV:-\(.*\)}$/\1/p' "$LIVE")
case " $LENV " in *" EXL3_MOE_PREFILL_E3_DET=1 "*) log "ABORT: DET already live"; exit 3;; *" EXL3_MOE_PREFILL_E3=1 "*) ;; *) log "ABORT: live env has no E3"; exit 3;; esac
...
# ask the image, not the build log: a cached rebuild prints none of the RUN output (R535 try 2)
sudo docker run --rm --entrypoint python3 "$NIMG" -c "import exllamav3.modules.block_sparse_mlp as b; from exllamav3.ext import exllamav3_ext as e; assert b.MOE_PREFILL_E3_DET is False; assert hasattr(e, 'exl3_moe_prefill_e3_det'); print('DET default off, kernel present')" >> "$R/build.log" 2>&1 \
  || { log "ABORT: image does not have the DET kernel with the flag default off"; exit 3; }
...
s=s[:m.start(1)]+m.group(1)+" EXL3_MOE_PREFILL_E3_DET=1"+s[m.end(1):]
open(dst,"w").write(s)
PY
...
export GPU_QUEUE_NAME=r535-promote-e3det
...
for f in EXL3_MOE_PREFILL_E3_DET=1 EXL3_HC_MIX_V2_INT8=1 EXL3_MOE_PREFILL_E3=1 EXL3_SHARED_EXPERT_OVERLAP=1 EXL3_MTP_HEAD_N=65536; do
...
log "=== PROMOTE: launch-flashnext.sh := launch-flashnext-r535.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r535
cp "$CAND" "$LIVE.new" && chmod 755 "$LIVE.new" && mv "$LIVE.new" "$LIVE"
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r535 "$LIVE"; finish ABORTED; exit 3; }
PROMOTED=1
sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || { log "LIVE LAUNCH FAILED — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r535 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
a2=$(greedy live); log "daily now: $(served_id) $(cfgline); c1 fingerprint $a2 (promotion-boot $a); VRAM free MiB $(vram)"
[ "$(served_id)" = "$NEWM" ] || { log "daily did not come up — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r535 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
finish PROMOTED
