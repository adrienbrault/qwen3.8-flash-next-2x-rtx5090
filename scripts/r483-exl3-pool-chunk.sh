#!/usr/bin/env bash
# R483 — exllamav3 pool past 360,448 at 8,8 / 4 slots: trade prefill chunk and split for pool.
#
...
#
# RUN: sudo systemd-run --unit=r483-exl3-pool-chunk --collect -p RuntimeMaxSec=43200 -E HOME=$HOME \
#        /bin/bash /srv/qwen5090/r483-exl3-pool-chunk.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-18-r483-exl3-pool-chunk; mkdir -p "$R"
...
SERVED=360448
BOOTED=0
log(){ echo "$(date -Is) [r483] $*" | tee -a "$R/audit.log"; }
...
finish(){ if [ "$BOOTED" = 1 ]; then log "restoring the daily (unchanged live launcher)"
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R483 $1 ==="; }
...
export GPU_QUEUE_NAME=r483-exl3-pool-chunk
...
# ctl: the served daily as found, decode + prefill only (the paired reference for those columns)
log "=== measure ctl (served, no reboot): $(cfgline) VRAM free $(vram) ==="
greedy ctl; decode ctl; prefill ctl

# PHASE 1
BEST=$SERVED; BEST_TAG=served; BEST_SPLIT="30, 30"; BEST_CHUNK=2048
POOLS="458752 425984 409600 393216 376832"
for combo in "C|30, 31|2048" "A|30, 30|1024" "D|30, 31|1024" "F|30.5, 31|1024" "B|30, 30|512" "E|30, 31|512"; do
  IFS='|' read -r tag split chunk <<< "$combo"
  for c in $POOLS; do
    [ "$c" -le "$BEST" ] && break
    try_boot "$tag" "$split" "$chunk" "$c"; rc=$?
    [ $rc = 2 ] && { finish ABORTED; exit 3; }
    [ $rc = 0 ] && { BEST=$c; BEST_TAG=$tag; BEST_SPLIT=$split; BEST_CHUNK=$chunk; break; }
  done
  log "after combo $tag: best pool $BEST ($BEST_TAG, split [$BEST_SPLIT], chunk $BEST_CHUNK)"
done
echo "$BEST|$BEST_TAG|$BEST_SPLIT|$BEST_CHUNK" > "$R/best.txt"

# PHASE 2
if [ "$BEST" -gt "$SERVED" ]; then
  if [ "$(served_id)" != "$MODEL" ] || ! cfgline | grep -q "cache_size: $BEST "; then
    try_boot "$BEST_TAG" "$BEST_SPLIT" "$BEST_CHUNK" "$BEST" || { log "ABORT: the winner does not re-boot"; finish ABORTED; exit 3; }
  fi
  measure "$BEST_TAG"
else log "no combo beats the served $SERVED"; fi
finish DONE
