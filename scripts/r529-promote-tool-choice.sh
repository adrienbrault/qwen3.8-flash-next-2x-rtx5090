#!/usr/bin/env bash
# R529 — promote TabbyAPI tool_choice enforcement (patches/tabbyapi/tool-choice/r1, round 3) on the Flash-Next daily.
# R523 round 3 (2026-09-19 02:30 UTC): 99 in-container tests OK; required 48/48, named 4/4, answer 24/24, validation 4/4,
# cap 2/2, concurrent 8/8, TC-45 x12 100 (the unpatched daily fails TC-45 12/12), 0 ToolChoiceNotHonoured, 0 loops, 50
# content-to-call continuations. Cold A/B identity is not judgeable from outputs on this engine: the same unpatched image on
# two fresh boots agreed on 3/12 cold tool-bearing requests (A vs B 3/12, A2 vs B 4/12; long tool prompts go through the
# non-deterministic E3 prefill); auto/none identity rests on the collector's unchanged branch and the 120-scenario CPU
# differential test (0 differences). Python-only overlay built on the LIVE image (it stacks on the pruned-draft image if
# that promoted): candidate launcher = live launcher with IMG := <live image>-tc1, nothing else. Gates: in-image tests;
# G1 boot + c1/30k fingerprints equal the live daily's (short prompts, auto path); decode floors against the live
# reference; agentic-edit; needles; tool-eval 69x4 >= 82 (the patch should lift it: TC-45); GSM8K >= 0.970. No VRAM
# change, so no G1v. PASS -> launch-flashnext.sh := candidate (old live kept as launch-flashnext.sh.pre-r529).
# RUN (queued): popped by r515-queue-chain from flashnext-queue.txt.
set -uo pipefail
export HOME=$HOME PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r529-promote-tool-choice; mkdir -p "$R"
...
CAND=/srv/qwen5090/launch-flashnext-r529.sh
...
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r529] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R529 $1 ==="; }
...
case "$LIMG" in *-tc1) log "ABORT: live image $LIMG already has tool_choice"; exit 3;; esac
NIMG="$LIMG-tc1"
...
grep -aE "^OK|^Ran " "$R/build.log" | tail -2 | tee -a "$R/audit.log"
...
export GPU_QUEUE_NAME=r529-promote-tool-choice
...
log "=== G2: agentic-edit ==="
python3 /srv/qwen5090/probes/agentic-edit.py --url "$API" --model "$NEWM" --tag TC --conc 1 4 --modes greedy sampled --out "$R/agentic-edit.jsonl" 2>&1 | tee "$R/agentic-edit.log" | grep -a "agentic-edit" | tee -a "$R/audit.log"
...
python3 /srv/qwen5090/probes/tooleval_summary.py "$R/tooleval.json" TC 2>&1 | tee "$R/tooleval-summary.txt" | tee -a "$R/audit.log"
...
log "=== PROMOTE: launch-flashnext.sh := launch-flashnext-r529.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r529
cp "$CAND" "$LIVE.new" && chmod 755 "$LIVE.new" && mv "$LIVE.new" "$LIVE"
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r529 "$LIVE"; finish ABORTED; exit 3; }
PROMOTED=1
sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || { log "LIVE LAUNCH FAILED — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r529 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
a2=$(greedy live); log "daily now: $(served_id) $(cfgline); c1 fingerprint $a2 (promotion-boot $a); VRAM free MiB $(vram)"
[ "$(served_id)" = "$NEWM" ] || { log "daily did not come up — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r529 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
finish PROMOTED
