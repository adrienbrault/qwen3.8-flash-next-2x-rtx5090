#!/usr/bin/env bash
# R446 — the five promotion gates on the PROMOTED prefill-pipeline daily (R442): the pipeline changes how long prompts are prefilled
# (windows + checkpoints), so the long-context gates (needle 131k, agent replay) are the ones that matter; c1/GSM8K/tool-eval = control.
# Derived from r414 (MAX_BSZN gates); the launcher defaults supply the four EXTRA_ENV keys.
...
TAG=${TAG:-ppipe}
R=${RDIR:-/srv/qwen5090/results/2026-09-17-r446-gates-$TAG}; mkdir -p "$R"
...
BOOTED=0
log(){ echo "$(date -Is) [r446] $*" | tee -a "$R/audit.log"; }
...
finish(){ if [ "$BOOTED" = 1 ]; then log "restoring the daily"; bash "$LIVE" >> "$R/audit.log" 2>&1 || log "RESTORE LAUNCHER FAILED"
    wait_id "$MODEL" && log "RESTORED: serving $MODEL on $(sudo docker ps --format '{{.Image}}' -f name=flashnext | head -1)" || log "RESTORE UNVERIFIED: $(served_id)"; fi; log "=== R446 $TAG $1 ==="; }
