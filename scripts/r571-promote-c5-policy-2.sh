#!/usr/bin/env bash
# R571 — second confirmation of the c5 draft policy P5 = [[4, 3], [5, 2], [8, 1]], aimed at the one clause R570 missed.
# R570 (4 boots A B B A, n 4 per arm per shape): c5 code +19.8 %, c5 prose +14.7 %, c4 +0.0 / -0.4 %, c6 code -0.0 %,
# c6 PROSE -2.1 % (rule floor -2 %), c8 -0.8 / +1.3 %; fingerprints canonical, boot free equal. R566 had c6 prose -1.8 % (n 2).
# Here: same ABBA, shapes c5 code (sanity) + c6 code / prose + c7 code / prose, 1,024 tokens, warm-up 1 + 4 runs (n 8 per arm).
#   Rule (fixed before boot): c5 code B/A >= +8 %; c6 code, c6 prose, c7 code, c7 prose each >= -2 %; the c6 prose verdict pools
#   R570's n 4 with this run's n 8 per arm (R570's records are read from its results dir). Fingerprints / headroom as R570.
#   Pass -> G1 tier ON -> PROMOTE early (.pre-r571) -> G2 agentic-edit c1 + c5, G3 needles, G4 tool-eval >= 82, G5 GSM8K >= 0.970
#   at 5 concurrent; any failure rolls back. Fail -> no promotion; c6 cost stays documented.
# GPU TIMEBOX ~1 h. RUN (queued): r515-queue-chain.
set -uo pipefail
export HOME=$HOME PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r571-promote-c5-policy-2; mkdir -p "$R"
...
CAND=/srv/qwen5090/launch-flashnext-r571.sh
...
SALT=$(( $(date +%s) % 100000 ))
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r571] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R571 $1 ==="; }
...
export GPU_QUEUE_NAME=r571-promote-c5-policy-2
...
bench(){ local tag=$1 conc kind
  for sh in 5:code 6:code 6:prose 7:code 7:prose; do conc=${sh%%:*}; kind=${sh#*:}
    python3 /srv/qwen5090/probes/fn_bench.py --url "$API" --model "$NEWM" --tag "$tag-c$conc-$kind" --kind $kind --tokens 1024 --warmup-runs 1 --conc $conc --runs 4 --out "$R/records.jsonl" 2>&1 \
      | grep -E "^  c=|FAILED|Traceback" | sed "s/^/[$tag c$conc $kind]/" | cut -c1-240 | tee -a "$R/audit.log"; done; }
...
python3 - "$R/ab.tsv" "$R/records.jsonl" /srv/qwen5090/results/2026-09-19-r570-promote-c5-policy/records.jsonl <<'PY' 2>&1 | tee "$R/analysis.txt" | tee -a "$R/audit.log"
...
def load(path):
    ...
    return agg
new=load(sys.argv[2]); old=load(sys.argv[3])
def arm(agg,a,conc,kind): return [x for t,v in agg.items() if t[0]==a and t.endswith(f"-c{conc}-{kind}") for x in v]
for conc,kind in ((5,"code"),(6,"code"),(6,"prose"),(7,"code"),(7,"prose")):
    ma=arm(new,"A",conc,kind); mb=arm(new,"B",conc,kind)
    ...
    if (conc,kind)==(6,"prose"):
        pa=ma+arm(old,"A",6,"prose"); pb=mb+arm(old,"B",6,"prose")
        d=st.mean(pb)/st.mean(pa)-1
        print(f"c6 prose pooled with R570: A {st.mean(pa):.1f} (n {len(pa)}) B {st.mean(pb):.1f} (n {len(pb)}) B/A {d*100:+.1f} %")
...
log "=== PROMOTE (early): launch-flashnext.sh := launch-flashnext-r571.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r571
cp "$CAND" "$LIVE.new" && chmod 755 "$LIVE.new" && mv "$LIVE.new" "$LIVE"
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r571 "$LIVE"; finish ABORTED; exit 3; }
...
rollback(){ log "POST-PROMOTION GATE FAILED ($1): rolling back to .pre-r571"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r571 "$LIVE"; PROMOTED=0; finish "ROLLED-BACK ($1)"; exit 3; }
log "=== G2: agentic-edit ==="
