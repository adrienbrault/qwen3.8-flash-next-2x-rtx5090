#!/usr/bin/env bash
# R507 — E3 grouped MoE prefill (codex prefill-e3-r1, after MiaAI-Lab's GLM-5.3-Flash kit; user 2026-09-18: "still worth trying").
# In a >= 512-row prefill chunk, experts with > 32 rows run a device-grouped 64x128x32 tile GEMM with fused dequant; thin experts
# stay on the served kernel; decode (<= 16 rows) never enters it. NOT bit-identical (accumulation order) -> GSM8K is the gate.
# IMAGE tabbyapi:prefill-e3-r1 = refbase (R504) + flan/patches/exllamav3/prefill-e3/r1 (GNU dry-run clean 2026-09-18 19:40).
# Launcher-r498 (= live r491 + CKPT_NAME), overlap on, pool 360,448.
#   gate 0  real-weight per-layer microbenchmark on both cards, rows 512 / 2048, OFF vs ON same input (codex bench): GO needs
#           2048-row ON faster on both cards and 512-row ON no more than 3 % slower; errors recorded (not an equality gate)
#   arms    OFF / ON / OFF2 / ON2 (EXL3_MOE_PREFILL_E3 0/1): c1 fingerprint canonical on every arm (101-token prompt < 512 rows);
#           30k fingerprint canonical on OFF arms, recorded on ON; cold prefill 30k / 60k / 120k x 2 (unique prompts); decode
#           fn_bench code + prose c1/c4 2,048 x 2 (must be unchanged)
#   quality GSM8K n=200 on OFF2 and ON2 (5-shot prompts are > 512 tokens, so ON exercises E3), needles 131k / 240k on ON2
# RUN: sudo systemd-run --unit=r507-prefill-e3 --collect -p RuntimeMaxSec=172800 -E HOME=$HOME /bin/bash /srv/qwen5090/r507-prefill-e3.sh
...
R=/srv/qwen5090/results/2026-09-18-r507-prefill-e3; mkdir -p "$R"
...
CTX=/srv/qwen5090/build/prefill-e3-r1
IMG=tabbyapi:prefill-e3-r1
...
BOOTED=0
log(){ echo "$(date -Is) [r507] $*" | tee -a "$R/audit.log"; }
...
finish(){ sudo docker rm -f r507-test >/dev/null 2>&1
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R507 $1 ==="; }
...
export GPU_QUEUE_NAME=r507-prefill-e3
...
print(f"[{sys.argv[2]}] GSM8K n=200 c4: flexible {s['exact_match,flexible-extract']:.3f} (SE {s['exact_match_stderr,flexible-extract']:.4f})   [served 0.925]")
PY
}
log "=== gate 0: real-weight per-layer microbenchmark, both cards, rows 512 / 2048 ==="
sudo docker run --rm --name r507-test --gpus all --ipc=host -v "$CKPT":/checkpoint:ro -v "$R":/results -e EXL3_MOE_PREFILL_E3=1 \
  --entrypoint python3 "$IMG" /opt/prefill-e3-r1/tests/bench_prefill_e3_layer.py --model /checkpoint --use-per-device 30,30 \
