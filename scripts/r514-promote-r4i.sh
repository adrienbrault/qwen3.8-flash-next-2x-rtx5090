#!/usr/bin/env bash
# R514 — promote codex decode round 4 group I (R499) on the 2.50bpw Flash-Next daily (:8022): image tabbyapi:decode-kernels-r4 with
# EXL3_DRAFT_PINNED_STAGING=1 EXL3_BATCH_VERIFY=1 EXL3_MTP_HEAD_N=65536 (candidate launcher launch-flashnext-r514.sh = r511 + those).
# R499: canonical greedy fingerprints on all four arms, code c1 +3.3 %, prose c1 +2.0 %, c4 +1.9-2.8 %, +120 MiB on cuda:1.
# Gates:
#   G0 sampler fallbacks (greedy + presence penalty, greedy + repetition penalty, greedy + stop strings) byte-identical to the live
#      daily (R499's probe crashed on TabbyAPI answers without a usage block; fixed here). Reference taken on the live daily first.
#   G1 boots at 786,432 with 4 slots / vision / chunk 2048, image decode-kernels-r4, the three flags in the container env;
#      greedy fingerprints MUST equal the canonical c1 ae890c45d1000582 / 30k 2aa8d1024daece5c (first 30k request of the boot).
#   G2 agentic-edit greedy + sampled, c1 + c4: 6/6 each, server alive
#   G3 needles 131k / 240k: 5/5 each
#   G4 tool-eval 69 x 4 at parallel 8: mean >= 82.0 (batched verify changes the sampled RNG stream; R511 control 86.0)
# PASS -> launch-flashnext.sh := r514 (old live kept as launch-flashnext.sh.pre-r514; rollback = launch-flashnext-r511.sh).
# RUN: sudo systemd-run --unit=r514-promote-r4i --collect -p RuntimeMaxSec=172800 -E HOME=$HOME /bin/bash /srv/qwen5090/r514-promote-r4i.sh
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-19-r514-promote-r4i; mkdir -p "$R"
API=http://127.0.0.1:8022/v1
NEWM=qwen3.8-flash-next-exl3-2.50bpw-r0b0tlab
LIVE=/srv/qwen5090/launch-flashnext.sh
CAND=/srv/qwen5090/launch-flashnext-r514.sh
CFG=/srv/qwen5090/flashnext-config.yml
PROMPT="Write a Python function that parses an ISO-8601 duration string into a datetime.timedelta, with tests. No explanation."
CLEAN_ENV=(env -i HOME=$HOME PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin)
BOOTED=0; PROMOTED=0
log(){ echo "$(date -Is) [r514] $*" | tee -a "$R/audit.log"; }
...
finish(){
  ...
  rm -f "${GPU_QUEUE_MARK:-/nonexistent}"; log "=== R514 $1 ==="; }
...
export GPU_QUEUE_NAME=r514-promote-r4i
...
[ "$img" = tabbyapi:decode-kernels-r4 ] || { log "G1 FAIL: image $img"; finish ABORTED; exit 3; }
for f in EXL3_DRAFT_PINNED_STAGING=1 EXL3_BATCH_VERIFY=1 EXL3_MTP_HEAD_N=65536 EXL3_SHARED_EXPERT_OVERLAP=1; do
...
[ "$a" = ae890c45d1000582 ] && [ "$b" = 2aa8d1024daece5c ] || { log "G1 FAIL: fingerprints not canonical"; finish ABORTED; exit 3; }
...
python3 /srv/qwen5090/probes/agentic-edit.py --url "$API" --model "$NEWM" --tag R4I --conc 1 4 --modes greedy sampled --out "$R/agentic-edit.jsonl" 2>&1 | tee "$R/agentic-edit.log" | grep -a "agentic-edit" | tee -a "$R/audit.log"
...
python3 /srv/qwen5090/probes/tooleval_summary.py "$R/tooleval.json" R4I 2>&1 | tee "$R/tooleval-summary.txt" | tee -a "$R/audit.log"
...
log "=== PROMOTE: launch-flashnext.sh := launch-flashnext-r514.sh ==="
sudo cp -p "$LIVE" /srv/qwen5090/launch-flashnext.sh.pre-r514
...
cmp -s "$CAND" "$LIVE" || { log "PROMOTE FAIL: live launcher differs from r514"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r514 "$LIVE"; finish ABORTED; exit 3; }
PROMOTED=1
sudo docker rm -f flashnext >/dev/null 2>&1
"${CLEAN_ENV[@]}" bash "$LIVE" >> "$R/audit.log" 2>&1 || { log "LIVE LAUNCH FAILED — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r514 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
for i in $(seq 120); do [ "$(served_id)" = "$NEWM" ] && break; sleep 2; done
a2=$(greedy); log "daily now: $(served_id) $(cfgline); c1 fingerprint $a2 (promotion-boot canonical $a)"
[ "$(served_id)" = "$NEWM" ] || { log "daily did not come up as $NEWM — rolling back"; sudo cp -p /srv/qwen5090/launch-flashnext.sh.pre-r514 "$LIVE"; PROMOTED=0; finish ABORTED; exit 3; }
finish PROMOTED
