...
| `docker/overlays/e3-det-r1/` | ExLlamaV3 (`exllamav3_ext/bindings.cpp`, `exllamav3_ext/quant/exl3_moe_prefill_e3.{cpp,cu,cuh}`, `modules/block_sparse_mlp.py`) | MIT | deterministic E3 grouped MoE prefill: per-assignment slots and a fixed-order reduction in place of `atomicAdd`, opt-in `EXL3_MOE_PREFILL_E3_DET=1`; written for this repository by an Opus agent round (R531, R533) |
