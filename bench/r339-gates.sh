...
# session reaches.
#
# RUN (probe only, no restart, but it holds the GPU lock so a daily restore cannot start mid-measurement):
...
#    do not all fit; what matters for a daily is whether the surplus queues or is rejected.
...
# 6. Stamina: a sustained c4 load. Watch for a decode-rate decay, an error rate, or VRAM drift; the poke every
#    round is what turns "it felt fine" into a number.
